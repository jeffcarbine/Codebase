...
const retrieveDatasets = () => {
      ...
      const datasetCard = renderTemplate(
        CARD({
          children: [
            ...
            new c.BTN({
              textContent: "Edit",
              href: "/periodic/admin/datasets/dataset/" + dataset._id,
            }),
          ],
        })
      );
  ...
  xhr({ path: "/periodic/admin/datasets/retrieve", callbacks: { success } });
};
...
const submitAddDatasets = (form) => {
  ...
  const formSuccess = (response) => {
    ...
    window.location = "/periodic/admin/datasets/dataset/" + dataset._id;
  };
