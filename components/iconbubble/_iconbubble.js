...
const toggleIconBubble = function (button, event) {
  ...
  const close = function (isOpen, className) {
    if (isOpen) {
      bubble.classList.remove(className);
    }
  };

  const open = function (isOpen, className) {
    if (!isOpen) {
...
const checkIconbubblePosition = function (iconbubble) {
